...
def createASC(shifts):
    
    # shifts = dictionary of lists indicating that shift to apply to the spectra for a given measurement time (list) of a given condition (dict)
    ...
    for meas_file in meas_file_names: 
        ...
        for i_file in range(len(fit_results_meas[:,0])): 
            ...
            shift = shifts[meas_file][i_file]                      
            wvn = wvn_full[2000:] + shift
